...
import nl.nlportal.zaken.client.request.ObjectType
...
class ZakenClientTest {
    lateinit var zakenConfigProperties: ZakenConfigProperties
    lateinit var catalogiApiConfigProperties: CatalogiApiConfigProperties
    lateinit var host: String
    lateinit var client: WebClient
    lateinit var mockServer: MockWebServer
    lateinit var zakenClient: ZakenClient
    ...
    fun setUp() {
        ...
        zakenConfigProperties =
            ZakenConfigProperties().apply {
                url = host
                clientId = "gzac"
        ...
        zakenClient = ZakenClient(zakenConfigProperties, catalogiApiConfigProperties, WebClient.builder())
    }
    ...
    fun `zaak url - generated correctly`() {
        val value = zakenClient.getZaakUrl(UUID.fromString("2941048a-7290-4a78-8c4e-14feaec137dc"))
    ...
    fun `zaken search - errors`() {
        ...
        return runTest {
            assertFailsWith<ResponseStatusException> {
                zakenClient.zaken().search().retrieve()
            }
        }
    }
    ...
    fun `zaken search - paging and retrieve all`() {
        ...
        return runTest {
            val response = zakenClient.zaken().search().retrieveAll()
    ...
    fun `zaken get - get single zaak`() {
        ...
        runTest {
            val response =
                zakenClient.zaken().get(UUID.fromString("2941048a-7290-4a78-8c4e-14feaec137dc")).retrieve()
